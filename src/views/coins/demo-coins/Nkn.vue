...
export default {
  name: 'Harmony',
  metaInfo: {
    title: 'NKN staking with Everstake',
    meta: [
      {
        vmid: 'description',
        name: 'description',
        content: 'Learn how to stake NKN to grow your NKN stake and yield. Start delegating NKN tokens with Everstake delegate today in just a few steps.',
      },
      {
        vmid: 'og:title',
        property: 'og:title',
        content: 'NKN staking with Everstake',
      },
      {
        vmid: 'og:description',
  ...
  data() {
    return {
      coinName: this.$helpers.capitalize(constants.COINS_MAP.nkn.name),
      coinIcon: Nkn,
      delegationAddress: constants.COINS_MAP.nkn.delegationAddress,
      fee: constants.COINS_MAP.nkn.fee,
    };
  },
  computed: {
    ...mapGetters({
    ...
    coinCode() {
      return this.$_getCoinCode('nkn').toUpperCase();
    },
  },
};
</script>
